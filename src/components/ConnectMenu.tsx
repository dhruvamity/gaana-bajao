...
export const ConnectMenu: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
      <div 
        className="w-full max-w-lg glass-elevated border border-white/15 rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl space-y-6 max-h-[85vh] overflow-y-auto"
      ...
      >
        ...
        <div className="p-4 rounded-2xl glass-panel border border-primary/20 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
        ...
        <div className="space-y-3">
          ...
          {sessions.filter(s => s.isCurrentDevice).map((device) => {
            ...
            return (
              <div
                key={device.id}
                className="p-4 rounded-2xl bg-primary/10 border border-primary/40 flex items-center justify-between"
              >
                <div className="flex items-center gap-3.5">
                  <div className="p-3 rounded-xl bg-primary text-on-primary shadow-lg shadow-primary/20">
                    <Icon size={20} />
                  </div>
                  <div>
            ...
            {sessions.filter(s => !s.isCurrentDevice).map((device) => {
              ...
              return (
                <button
                  ...
                  className="w-full p-3.5 rounded-2xl glass-panel border border-white/5 hover:border-primary/40 hover:bg-white/5 flex items-center justify-between text-left transition-all group"
                >
                  <div className="flex items-center gap-3.5">
                    <div className="p-2.5 rounded-xl glass-subtle text-on-surface-variant group-hover:text-primary transition-colors">
                      <Icon size={18} />
                    </div>
                    <div>
